...
main = do
  hspec $
    describe "src/Parser.hs" $ do
      ...
      it "Nil" $
        readExpr "'()" `shouldBe` (Right $ List [Atom "quote", Nil])

      it "S-Expr: homogenous list" $
        readExpr "(2 1 87)"
          `shouldBe` (Right $ List [Number 2, Number 1, Number 87])

      it "S-Expr: homogenous list quoted" $
        readExpr "'(2 1 87)"
          `shouldBe` (Right $ List [Atom "quote", List [Number 2, Number 1, Number 87]])

      it "S-Expr: heterogenous list" $
        readExpr "(stromTrooper \"Fn\" 2 1 87)"
          `shouldBe` (Right $ List [Atom "stromTrooper", String "Fn", Number 2, Number 1, Number 87])

      it "S-Expr: heterogenous list quoted" $
        readExpr "'(stromTrooper \"Fn\" 2 1 87)"
          `shouldBe` (Right $ List [Atom "quote", List [Atom "stromTrooper", String "Fn", Number 2, Number 1, Number 87]])
      ...
      it "S-Expr: prim call: numbers" $
        readExpr "(+ 1 2)"
          `shouldBe` (Right $ List [Atom "+", Number 1, Number 2])

      it "S-Expr: prim call: neg nums" $
        readExpr "(- -42 -42)"
          `shouldBe` (Right $ List [Atom "-", Number (-42), Number (-42)])

      it "S-Expr: prim call: atoms" $
        readExpr "(- rogue squadron)"
          `shouldBe` (Right $ List [Atom "-", Atom "rogue", Atom "squadron"])

      it "S-Expr: nested list" $
        readExpr "(lambda (x x) (+ x x))"
          `shouldBe` (Right $ List [Atom "lambda", List [Atom "x", Atom "x"], List [Atom "+", Atom "x", Atom "x"]])
